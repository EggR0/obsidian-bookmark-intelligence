...
이 updater는 GitHub의 `latest` Windows/source Release asset을 임시 폴더에 내려받은 뒤 현재 설치 디렉터리에 반영하고 설치합니다. 따라서 Native Messaging manifest가 임시 폴더를 가리키지 않으며, 기존 `config.toml`과 Vault Markdown도 보존됩니다. 브라우저 스토어 확장은 각 스토어의 심사·게시 후 스토어 자동 업데이트 정책을 따릅니다. 설치 중 실행 중인 worker는 기존 프로세스가 계속 실행될 수 있으므로, 업데이트 후 worker를 한 번 재시작하는 것이 권장됩니다.
