...
결제·로그인 entitlement 서버의 최소 구현은 `server/billing_service.py`에 있습니다. 계정 등록/로그인, bearer token, 결제 주문 매핑, Polar Standard Webhooks 검증, Toss 결제 API 재조회, 중복 이벤트 차단을 제공합니다. 실제 공개 서비스로 운영할 때는 HTTPS reverse proxy, rate limiting, email verification, password reset, managed database, secret manager를 추가해야 합니다. 상세 실행법은 [`server/README.md`](server/README.md)를 참조합니다.
